package bd;

import java.util.Calendar;

import org.json.JSONObject;

public class BdStatic {

	//info statique sur la bd
	public final static boolean mysql_pooling=false;
	public final static String  mysql_host="localhost:3306";//"http://localhost/phpmyadmin"; //
	public final static String  mysql_db="hacene_anis";
	public final static String  mysql_username="root";
	public final static String  mysql_password="root";
	public final static String  mongo_host="mongodb://localhost:27017";
	public final static String  mongo_db="DataBaseMongo_Anis_Hacene";
	
}
